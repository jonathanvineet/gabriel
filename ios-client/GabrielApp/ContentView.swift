...
struct DashboardView: View {
    ...
    @State private var weather = WeatherData(temp: 22, condition: "Clear", location: "Gotham City")
            ...
            VStack(spacing: 20) {
                ...
                if checkingConnection {
                    ...
                    if horizontalSizeClass == .regular {
                        ...
                        // Camera Feed - Full Width on iPad
                        CameraFeedCard()
                            .frame(height: 400)
                            .padding(.horizontal, 24)
                        
                    } else {
                        // iPhone Layout: 2-column grid
                        LazyVGrid(columns: gridColumns, spacing: 16) {
    ...
    class MJPEGStreamView: NSObject, ObservableObject, URLSessionDataDelegate {
        ...
        func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
            if let error = error {
                ...
                print("MJPEG stream completed normally")
                DispatchQueue.main.async {
                    self.isStreaming = false
                }
            }
