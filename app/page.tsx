      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 max-w-7xl relative z-10">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="text-right">
              ...
              <p className="text-gray-500 text-sm uppercase tracking-wider mb-2">{currentTime.toLocaleTimeString('en-US', { hour12: true }).split(' ')[1]}</p>
              {/* Weather info inline */}
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            {/* Recent Files Card */}
            <div className="col-span-12 md:col-span-4 batman-card p-8 hover:border-amber-400/50 transition-all duration-300">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-white/60 uppercase text-sm font-bold tracking-widest">Recent Files</h3>
                <FileText className="w-6 h-6 text-amber-400" />
              </div>
              
              {recentFiles.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-white/30 text-sm">No recent files</p>
                </div>
              ) : (
                <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                  {recentFiles.slice(0, 8).map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center gap-3 p-3 bg-black/20 border border-white/5 rounded-xl hover:border-amber-400/30 transition-all cursor-pointer group"
                      onClick={() => handleImagePreview(file)}
                    >
                      <div className="w-10 h-10 bg-black/30 rounded-lg overflow-hidden flex items-center justify-center flex-shrink-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-white text-sm truncate group-hover:text-amber-400 transition-colors">{file.name}</p>
            ...
            {/* USB Camera Feed */}
                      {/* Camera */}
          <div className="col-span-8 batman-card p-6">
            ...
            {/* Things To Do */}
            <div className="col-span-12 md:col-span-6 batman-card p-8 hover:border-amber-400/50 transition-all duration-300">
              <div className="flex items-center justify-between mb-6">
        ...
        <div className="mt-8">
          {!isStorageExpanded ? (
            /* Storage Preview */
            <div 
              className="batman-card p-8 hover:border-amber-400/50 transition-all duration-300 cursor-pointer"
